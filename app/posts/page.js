  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="max-w-2xl mx-auto p-4">
        ...
        {posts.length === 0 ? (
          <div className="text-center text-gray-500 py-8"><p className="text-lg mb-2">No posts to show!</p></div>
        ) : (
          posts.map((post) => (
            <div key={post.id} className="bg-white rounded-lg shadow p-4 mb-4">
              <div className="flex items-center justify-between mb-3">
                <Link href={`/profile/${post.user_id}`} className="flex items-center hover:opacity-80 transition">
                  ...
                  <div>
                    <p className="font-semibold text-gray-800 hover:underline">{post.profiles?.full_name || 'Unknown'}</p>
                ...
                {post.user_id === user.id && (
                  <div className="relative">
                    <button 
                      ...
                      className="text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full w-8 h-8 flex items-center justify-center text-xl leading-none"
                    >
                      ···
                    </button>
                    {menuOpen === post.id && (
                      <div className="absolute right-0 top-9 bg-white shadow-lg rounded-lg py-1 z-20 border min-w-[130px]">
              ...
              <div className="border-t pt-3">
                {post.comments.map((comment) => (
                  <div key={comment.id} className="mb-2 text-sm flex justify-between items-start">
                    <div>
                      <Link href={`/profile/${comment.user_id}`} className="font-semibold text-gray-800 hover:underline">{comment.profiles?.full_name || 'Unknown'}</Link>
